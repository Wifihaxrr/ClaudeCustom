//#define DEBUG
// PathFinding is optional - NPCs will work but without advanced pathfinding
...
namespace Oxide.Plugins
{
    ...
    public class HumanNPC : RustPlugin
    {
        ...
        private static Plugin PathFinding;
                ...
                else
                {
                    ...
                    foreach (var cwaypoint in (List<object>)cwaypoints)
                    {
                        foreach (var pair in (Dictionary<Vector3, float>)cwaypoint)
                        {
                            ...
                            var temppathFinding = HumanNPC.PathFinding?.Call<List<Vector3>>("Go", lastPos, pair.Key);
                            speed = pair.Value;
                    ...
                    {
                        var temppathFinding = HumanNPC.PathFinding.Call<List<Vector3>>("Go", lastPos, npc.info.spawnInfo.position);
                        if (temppathFinding != null)
                        {
                            foreach (var vector3 in temppathFinding)
                                cachedWaypoints.Add(new WaypointInfo(vector3, speed));
...
//                    npc.Invoke("AllowMove",0);
            ...
            public void PathFinding(Vector3 targetPos)
            {
                ...
                var temppathFinding = HumanNPC.PathFinding?.Call<List<Vector3>>("Go", npc.player.transform.position, targetPos);
